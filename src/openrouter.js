...
export async function complete({ apiKey, model, messages, accept = (t) => t, attempts = 5, backoff = 15000 }) {
  let last = "no attempt made";
  let nudge = false;
  for (let i = 0; i < attempts; i++) {
    if (i) {
      const wait = backoff * i;
      console.log(`openrouter attempt ${i} failed (${last}), retrying in ${wait / 1000}s`);
      await sleep(wait);
    }
